                ...
                >
                    ...
                    <div className="flex flex-col md:flex-row">
                        ...
                        <div className="w-full md:w-1/2 p-6 md:p-8 flex flex-col gap-6">
                            <motion.div variants={itemVariants}>
                                <h3 className="text-xl font-semibold text-slate-800 dark:text-white mb-3">Technologies</h3>
                                <div className="flex flex-wrap gap-2">
                                    {project.tags.map((tag, idx) => (
                                        <span key={idx} className="px-3 py-1 text-sm bg-primary/10 text-primary rounded-full font-medium">
                                            {tag}
                                        </span>
                                    ))}
                                </div>
                            ...
                            <motion.div variants={itemVariants}>
                                <h3 className="text-xl font-semibold text-slate-800 dark:text-white mb-2">Description</h3>
                            ...
                            >
                                {project.live && (
                                    <a href={project.live} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 py-2 px-4 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors font-medium">
                                        <RxOpenInNewWindow size={20} /> Live Demo
                                    </a>
                                )}
                                {project.githubClient && (
                                    <a href={project.githubClient} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 py-2 px-4 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors font-medium">
                                        <FaGithub size={20} /> Client Repo
                                    </a>
                                )}
                                {project.githubServer && (
                                    <a href={project.githubServer} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 py-2 px-4 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors font-medium">
                    ...
                    <div className="p-6 md:p-8 border-t border-slate-100 dark:border-slate-800 grid md:grid-cols-2 gap-8">
                        <motion.div variants={itemVariants}>
                            ...
                            <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
                                {project.challenges || "Details about the challenges faced during the development of this project."}
                            </p>
                        </motion.div>
                        <motion.div variants={itemVariants}>
                            <h3 className="text-xl font-semibold text-slate-800 dark:text-white mb-3 flex items-center gap-2">
                            ...
                            <p className="text-slate-600 dark:text-slate-400 leading-relaxed">
                                {project.futurePlans || "Upcoming features and improvements planned for future updates."}
                            </p>
                        </motion.div>
                    </div>
                </motion.div>
...
const ProjectCard = ({ title, description, tags, image, type, index, live, githubClient, githubServer, onOpen }) => {
        ...
        >
            ...
            <div className="relative h-full bg-white dark:bg-slate-900 rounded-2xl overflow-hidden flex flex-col z-10">
                ...
                <div className="p-6 flex flex-col flex-grow">
                    <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2 line-clamp-1" title={title}>{title}</h3>
                    <p className="text-slate-600 dark:text-slate-400 text-sm mb-4 line-clamp-3">{description}</p>

                    <div className="flex flex-wrap gap-2 mb-6 mt-auto">
                        {tags.slice(0, 3).map((tag, idx) => (
                            <span key={idx} className="text-xs font-medium px-2 py-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded">
                                {tag}
                            </span>
                        ))}
                        {tags.length > 3 && (
    ...
    const projects = [
        {
            ...
            challenges: 'Implementing secure role-based access control (RBAC) with JWT and managing complex database relationships between users, scholarships, and applications was challenging. Integrating Stripe for seamless payments also required careful handling of webhooks.',
            futurePlans: 'I plan to add an AI-based recommendation engine to suggest scholarships to students based on their profile. Mobile responsiveness improvements and multi-language support are also in the pipeline.'
        },
        {
            ...
            challenges: 'Handling real-time image uploads and optimizing them for fast loading speeds was a key hurdle. Creating a dynamic and responsive filter system for property searching required complex query logic on the backend.',
            futurePlans: 'Integration of a real-time chat feature between buyers and sellers using Socket.io. I also want to add a map view using Google Maps API to show property locations.'
        },
        {
            ...
            githubServer: '',
            challenges: 'Ensuring a visually appealing and child-friendly design while maintaining professional e-commerce functionality. Managing local state for the cart and protecting routes based on user authentication were also interesting challenges.',
            futurePlans: 'I aim to convert this into a full-stack application with a backend for inventory management. Adding a "Wishlist" feature and user reviews for products is also planned.'
        },
        {
            ...
            challenges: 'Learning and utilizing Next.js 16 features like Server Actions and the new routing system. Configuring authentication securely with NextAuth (or custom solution) alongside protected API routes.',
            futurePlans: 'Implementing a payment gateway (like SSLCommerz or Stripe). Adding a blog section for product reviews and SEO optimization to drive organic traffic.'
        },
        {
            title: 'HERO.IO',
            ...
            githubServer: '',
            challenges: 'Building a responsive grid layout that mimics a native app store experience on the web. Handling large localized datasets and implementing efficient search and sort functionality on the client side.',
            futurePlans: 'Fetching dynamic data from a real external API. Implementing PWA (Progressive Web App) features so users can install it on their devices.'
        }
    ];
